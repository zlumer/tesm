...
Let's create our state transition logic by passing the object to the **`enhanceMachine()`** function as its 4th argument.
...
The `enhanceMachine` function will throw an error if current state cannot handle the message.
